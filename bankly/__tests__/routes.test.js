...
const tokens = {bad: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6Im15dGVzdCIsImFkbWluIjpmYWxzZSwiaWF0IjoxNjg0NDUyMDYxfQ.M1nPLUlMxEnipYu47JEh2r_qnclZIbFTvUoiXN2PdFQ'};
...
describe("POST /auth/login", function() {
  ...
  // ****************************** TESTS BUG #4 ******************************
  test("should fail for incorrect username/password to log in", async function() {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u1",
...
describe("GET /users", function() {
  ...
  // ****************************** TESTS BUG #1 ******************************
  test("should fail if corrupted token is inserted", async () => {
      const response = await request(app)
      .get("/users")
      .set("Authorization", `Bearer ${tokens.bad}`);
    expect(response.statusCode).toBe(401);
...
describe("GET /users/[username]", function() {
  ...
  // ****************************** TESTS BUG #6 ******************************
  test("should fail if user does not exist", async function() {
    const response = await request(app)
      .get("/users/u4")
      .set("Authorization", `Bearer ${tokens.u1}`);
...
describe("PATCH /users/[username]", function() {
  ...
  // ****************************** TESTS BUG #2 ******************************
  test("should allow access if correct user", async () => {
    const response = await request(app)
      .patch("/users/u1")
      .set("Authorization", `Bearer ${tokens.u1}`)
...
describe("DELETE /users/[username]", function() {
  ...
  test("should fail if user does not exist", async function() {
    const response = await request(app)
      ...
      .set("Authorization", `Bearer ${tokens.u3}`);
    expect(response.statusCode).toBe(404);
    expect(response.body.message).toEqual("No such user");
  });
});
