...
const { requireLogin, requireAdmin, requireCorrectUser } = require('../middleware/auth');
...
router.patch('/:username', requireCorrectUser, async function(req, res, next) {
  try {
    ...
    Object.keys(req.body).map(key => {
      const acceptableKeys = ['first_name', 'last_name', 'phone', 'email']
      if (acceptableKeys.indexOf(key) === -1) {
        throw new ExpressError('Only an admin can edit that.', 401);
      }
    })
...
router.delete('/:username', requireAdmin, async function(req, res, next) {
  try {
    // ****************************** FIXES BUG #4 ******************************
    // Following command was missing "await"
